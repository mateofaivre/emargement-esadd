...
<nav class="navbar">
    <ul class="navbar-nav">
      <li class="logo">
        <a href="#" class="nav-link">  <?php if (isset($_SESSION['photoUser'])){
  ...
  <h1>Admin</h1>
    ...
    ?><p class="doneMessage"> <?php  echo $_SESSION['done_message']; ?> </p><?php
        ...
        <form action="droits-create-action.php<?php  if(isset($_GET["action"], $_GET["id"]) && $_GET["action"]=="edit" && is_numeric($_GET["id"])){ echo "?id=".$id; } ?>" method="post" enctype="multipart/form-data">
            ...
            if(isset($_GET["action"], $_GET["id"]) && $_GET["action"]=="edit" && is_numeric($_GET["id"])){
                $stmt_d = $dbh->prepare("SELECT idUser, nomUser, prenomUser, mailUser, codeUser, signatureUser, photoUser, isAdmin, isProf, isActualUser FROM user WHERE isAdmin = 1 AND idUser='$id'");
            ...
            <div id="containerDatas">
                        ...
                        <p>(L'utilisateur doit signer lui-même.)</p>
                        <img id="sig-image" name="sig-image" src="<?php  echo $donnees_d['signatureUser']; ?>" alt="Vous devez demander au professeur de signer lui-même !" onerror="document.querySelector('#sig-image').style.display='none'; document.querySelector('.clearSign').style.display='none'; document.querySelector('.containerSignature p').style.display='block'"/>
                    </div>
            </div>
           <?php } ?>
            ...
            <div class="containerCheckbox">
              ...
              ?> <label for="checkActual">Utilisateur actuel</label> <?php
        
              if ($donnees_d['isProf'] == 1) { ?>
                  <input type='checkbox' name="checkProf" class="check" id='checkProf' checked>
             <?php }
             else { ?>
                    <input type='checkbox' name="checkProf" class="check" id='checkProf'>
             <?php } 

                ?>  <label for="checkProf">Professeur</label> <?php
                } 
              else { ?>
                <input type='checkbox' name="checkActual" class="check" id='checkActual'>
                <label for="checkActual">Utilisateur actuel</label>
                <input type='checkbox' name="checkProf" class="check" id='checkProf'>
                <label for="checkProf">Professeur</label>
              ...
              <input type="password" id="txtPassword" name="user_code" placeholder="Mot de passe" value="<?php   if(isset($_GET["action"], $_GET["id"]) && $_GET["action"]=="edit" && is_numeric($_GET["id"])){echo $donnees_d['codeUser'];}
              else {randomPassword();} ?>" required>
