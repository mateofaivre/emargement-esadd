...
function checkPhoto($nom, $prenom , $mail, $code, $signature, $photoName, $photoTmp){
    ...
    if ($photoName != ""){
        if($_FILES['user_file']['error'] != UPLOAD_ERR_OK) {
            ...
            if (isset($id)){
                header("Location: eleves-create.php?action=edit&id=$id");
            }
            else {
                header("Location: eleves-create.php");
            }
        }
        ...
        if ($size > (10000 * 1024)) {
            ...
            if (isset($id)){
                $stmt_s = $dbh->prepare("UPDATE user SET nomUser = '$nom', prenomUser = '$prenom', mailUser = '$mail', codeUser = '$code', signatureUser = '$signature', photoUser = '$Image', isActualUser = '$isActualUser' WHERE idUser = $id");
                ...
                $stmt_s = $dbh->prepare("INSERT INTO user (nomUser, prenomUser, mailUser, codeUser, signatureUser, photoUser, isProf, isAdmin, isActualUser) VALUES ('$nom', '$prenom', '$mail', '$code', '$signature', '$Image', '0', '0', '$isActualUser' )");
                $donnees_s = $stmt_s->execute();
                $_SESSION['error_message'] = "";
        ...
        if (isset($id)){
            $stmt_s = $dbh->prepare("UPDATE user SET nomUser = '$nom', prenomUser = '$prenom', mailUser = '$mail', codeUser = '$code', signatureUser = '$signature', isActualUser = '$isActualUser' WHERE idUser = $id");
            $donnees_s = $stmt_s->execute();
            $_SESSION['error_message'] = "";
            $_SESSION['done_message'] = "Les modifications ont bien été effectuées.";
            header("Location: eleves-create.php?action=edit&id=$id");
            ...
            echo $photoName;
            $stmt_s = $dbh->prepare("INSERT INTO user (nomUser, prenomUser, mailUser, codeUser, signatureUser, isProf, isAdmin, isActualUser) VALUES ('$nom', '$prenom', '$mail', '$code', '$signature', '0', '0', '$isActualUser' )");
            $donnees_s = $stmt_s->execute();
            $_SESSION['error_message'] = "";
            $_SESSION['done_message'] = "L'utilisateur a bien été créé.";
            header("Location: eleves-create.php");
