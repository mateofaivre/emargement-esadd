...
function checkPhoto($nom, $prenom , $mail, $code, $signature, $photoName, $photoTmp){
    ...
    if ($photoName != ""){
        if($_FILES['user_file']['error'] != UPLOAD_ERR_OK) {
            ...
            if (isset($id)){
                header("Location: prof-create.php?action=edit&id=$id");
            }
            else {
                header("Location: prof-create.php");
            }
        }
        ...
        if ($size > (10000 * 1024)) {
            ...
            if (isset($id)){
                ...
                $stmt_s = $dbh->prepare("INSERT INTO user (nomUser, prenomUser, mailUser, codeUser, signatureUser, photoUser, isProf, isAdmin, isActualUser) VALUES ('$nom', '$prenom', '$mail', '$code', '$signature', '$Image', '1', '0', '$isActualUser' )");
                $donnees_s = $stmt_s->execute();
                $_SESSION['error_message'] = "";
        ...
        if (isset($id)){
            ...
            $stmt_s = $dbh->prepare("INSERT INTO user (nomUser, prenomUser, mailUser, codeUser, signatureUser,  isProf, isAdmin, isActualUser) VALUES ('$nom', '$prenom', '$mail', '$code', '$signature', '1', '0', '$isActualUser' )");
            $donnees_s = $stmt_s->execute();
            $_SESSION['error_message'] = "";
            $_SESSION['done_message'] = "L'utilisateur a bien été créé.";
            header("Location: prof-create.php");
