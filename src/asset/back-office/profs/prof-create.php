...
<nav class="navbar">
  ...
  <h1>Professeur</h1>
    ...
    ?><p class="doneMessage"> <?php  echo $_SESSION['done_message']; ?> </p><?php
        ...
        <form action="profs-create-action.php<?php  if(isset($_GET["action"], $_GET["id"]) && $_GET["action"]=="edit" && is_numeric($_GET["id"])){ echo "?id=".$id; } ?>" method="post" enctype="multipart/form-data">
            ...
            if(isset($_GET["action"], $_GET["id"]) && $_GET["action"]=="edit" && is_numeric($_GET["id"])){
                $stmt_d = $dbh->prepare("SELECT idUser, nomUser, prenomUser, mailUser, codeUser, signatureUser, photoUser, isActualUser FROM user WHERE isProf = 1 AND idUser='$id'");
            ...
            <div id="containerDatas">
                        ...
                        <p>(Le professeur doit signer lui-même.)</p>
           ...
           <?php } ?>
            ...
            <div class="containerCheckbox">
              ...
              <label for="checkActual">Enseigne actuellement</label>
            </div>
