...
<body>
    ...
    <?php
        ...
        if(isset($_GET["action"], $_GET["id"]) && $_GET["action"]=="delete" && is_numeric($_GET["id"])){
            ...
            $stmtGroupeUser = $dbh->prepare("DELETE FROM groupeuser WHERE idGroupe='$id'");
            $donneesGroupeUser = $stmtGroupeUser->execute();
            $stmtGroupe = $dbh->prepare("DELETE FROM groupe WHERE idGroupe='$id'");
            $donneesGroupe = $stmtGroupe->execute();
            $_SESSION['error_message'] = "";
            $_SESSION['done_message'] = "L'utilisateur a bien été supprimé.";
        }
        $stmt = $dbh->prepare("SELECT idGroupe, nomGroupe, idPromo FROM groupe ORDER by nomGroupe");
    ...
    ?><p class="doneMessage"> <?php  echo $_SESSION['done_message']; ?> </p><?php
        ...
        <div id="containerList">
          ...
          <div class="containerTools">
            <h2 class="hRole">Groupes</h2>
            <a href="groupes-create.php">Ajouter</a>
            <div class="filtres">
                <input class="search" placeholder="&#xF002;" />
                <div class="filtresSort">
                  <span class='sort' data-sort='nomGroupes' data-default-order='asc'>Groupes</span>
                </div>
              </div>
          </div>
        <ul class="list">
        ...
        ?>
            ...
            <li class="liList">
              <div class="containerGroupes nomGroupes">
                <?php echo $donnees['nomGroupe']?>
                <span class="ids"><?php echo $donnees['idGroupe']?></span>
              </div>
              <div class="containerActions">
              <a href="groupes-create.php?action=edit&amp;id=<?php echo $donnees['idGroupe']?>&groupe=<?php echo $donnees['nomGroupe'];?>&idPromo=<?php echo $donnees['idPromo']?>"><img src="../../../img/Icons/Edit.svg" alt=""></a>
      ...
      <div id="modalWindow" class="modal">
            <div class="modal-content">
                <p>Confirmez-vous la suppression de <span id="nomGroupeModal"></span> ? </p>    
              
                <div class="actionBtns">
                    <a href="#"  name="annuler" id="annulerModal" class="btnModal">Annuler</a>
                    <a href="groupes.php?action=delete&amp;id=  " name="valider" id="validerModal" class="btnModal">Valider</a>
                </div>
